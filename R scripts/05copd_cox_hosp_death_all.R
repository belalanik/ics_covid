...
# Date: 03/2023
# Purpose: generate propensity scores and IPTW weights for COPD cohort. Fit Cox models for all three outcomes.USes the cohort that includes people with asthma, other chronic resp disease.
...
df <- read_parquet("SA_copd_wave1_60d_iptw_all.parquet")
...
write.xlsx(censored, file.path(Tables, "SA_censored_table_all.xlsx"), rowNames = T)
...
for (j in seq_along(outcomes)){
  ...
  file_path <- file.path(Graphdir, "cox_regression", paste0("SA_schoenfeld_resid_", outcome_event, "_unadj_all.png"))
  png(file_path)
  ...
  file_path_resid <- file.path(Graphdir, "cox_regression", paste0("SA_log_resid_", outcome_event, "_unadj_all_jittered.png"))
  png(file_path_resid)
  ...
  for (w in seq_along(weight_vars)) {
    ...
    file_path_iptw <- file.path(Graphdir, "cox_regression", paste0("SA_schoenfeld_resid_", outcome_event, "_IPTW_", weight_name, "_all.png"))
    png(file_path_iptw)
    ...
    file_path_resid <- file.path(Graphdir, "cox_regression", paste0("SA_log_resid_", outcome_event, "_IPTW_all_", weight_name, "_jittered.png"))
...
arrow::write_parquet(estimates, file.path(Tables, "QBA", "SA_cox_log_regression_estimates_all.parquet"))
