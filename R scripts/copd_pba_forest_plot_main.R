...
estimates_conventional <- read_parquet(file.path(Tables, "QBA", "cox_log_regression_estimates.parquet")) %>% dplyr::select(-contains("coef_"), -contains("se_"), -contains("res_p"), -contains("cox"), -contains("hr"), -contains("att_weight")) %>% filter(!stringr::str_detect(outcome_event, "any_"))

hosp_summary <- read_parquet(file.path(Tables, "QBA", "qba_hosp_summary_results.parquet"))
hosp_record <- read_parquet(file.path(Tables, "QBA", "qba_hosp_record_results.parquet"))
death_summary <- read_parquet(file.path(Tables, "QBA", "qba_death_summary_results.parquet"))
death_record <- read_parquet(file.path(Tables, "QBA", "qba_death_record_results.parquet"))

#load in death iptw dataset
D <- read_parquet(file.path(Datadir_copd, "copd_wave1_60d_iptw.parquet"))
...
file_path_hosp <- file.path(Graphdir, "QBA", "forest_plot_full_main.png")
