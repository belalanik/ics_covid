...
estimates_conventional <- read_parquet(file.path(Tables, "QBA", "SA_cox_log_regression_estimates_all.parquet")) %>% dplyr::select(-contains("coef_"), -contains("se_"), -contains("res_p"), -contains("cox"), -contains("hr"), -contains("att_weight")) %>% filter(!stringr::str_detect(outcome_event, "any_"))

hosp_summary <- read_parquet(file.path(Tables, "QBA", "qba_hosp_summary_results_all.parquet"))
hosp_record <- read_parquet(file.path(Tables, "QBA", "qba_hosp_record_results_all.parquet"))
death_summary <- read_parquet(file.path(Tables, "QBA", "qba_death_summary_results_all.parquet"))
death_record <- read_parquet(file.path(Tables, "QBA", "qba_death_record_results_all.parquet"))

#load in death iptw dataset
D <- read_parquet(file.path(Datadir_copd, "SA_copd_wave1_60d_iptw_all.parquet"))
...
df <- read_parquet("copd_wave1_60d_hosp_all.parquet")
DH <- read_parquet("SA_copd_wave1_60d_iptw_all.parquet")[, c("patid", "ate_weight_stab")]
...
file_path_hosp <- file.path(Graphdir, "QBA", "forest_plot_full_main_all.png")
