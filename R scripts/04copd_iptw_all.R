...
packages <- c("tidyverse", "MetBrewer", "arrow", "MatchThem", "mice", "survey", "cobalt", "ggplot2", "twang", "ipw", "WeightIt", "gtsummary", "flextable", "openxlsx", "jskm", "tidysmd", "tools")
...
df <- read_parquet("copd_wave1_60d_all.parquet")
...
subset_df <- subset_df[, c("patid", "age_index", "gender", "death_date", "imd", "bmicat", "eth", "diabetes_present", "hypertension_present", "cvd_present", "allcancers_present", "past_asthma_present", "current_asthma_present", "kidney_present", "immunosuppression_present", "flu_vacc_present", "pneumo_vacc_present", "exacerb_present", "smokstatus", "timeinstudy1", "timeinstudy2", "timeinstudy3", "timeinstudy_death_any", "timeout1", "timeout2", "timeout3", "timeout_death_any", "pos_covid_test_present", "covid_hes_present", "covid_death_present", "any_death_present", "treat", "treatgroup", "missing_ons")]

#check missingness patterns
md.pattern(subset_df, rotate.names = TRUE)

#check number of missings per variable in model
missing_counts <- colSums(is.na(subset_df[, c("age_index", "gender", "imd", "bmicat","eth", "diabetes_present", "hypertension_present", "cvd_present", "allcancers_present", "past_asthma_present", "current_asthma_present", "kidney_present", "immunosuppression_present", "flu_vacc_present", "pneumo_vacc_present", "exacerb_present", "smokstatus")]))
missing_counts 
...
ps_formula <- treat ~ age_index + gender + eth + imd + bmicat + diabetes_present + hypertension_present + cvd_present + allcancers_present + past_asthma_present + current_asthma_present + kidney_present + immunosuppression_present + flu_vacc_present + pneumo_vacc_present + exacerb_present + smokstatus

# Generating propensity scores as specified in Austin (2011)
ps1 <- glm(treatgroup ~ age_index + gender + eth + imd + bmicat + diabetes_present + hypertension_present + cvd_present + allcancers_present + past_asthma_present + current_asthma_present + kidney_present + immunosuppression_present + flu_vacc_present + pneumo_vacc_present + exacerb_present + smokstatus,
           data = subset_df,
           family = "binomial")
...
model_rows <- c("Intercept", "Age at baseline", "Gender: Female", "Ethnicity: South Asian", "Ethnicity: Black", "Ethnicity: Mixed", "Ethnicity: Unknown", "IMD 2", "IMD 3", "IMD 4", "IMD 5", "IMD: Missing",  "BMI: Underweight (< 18.5)", "BMI: Overweight (25-29.9)", "BMI: Obese (>=30)", "Diabetes", "Hypertension", "CVD", "Cancer", "Past asthma", "Current asthma", "Chronic Kidney Disease", "Immunosuppression", "Influenza Vaccine (past 12 months)", "Pneumococcal Vaccine (past 5 years)", "COPD exacerbation (past 12 months)", "Former smoking")
...
file_path <- file.path(Graphdir, "iptw_diagnostics", "SA_ps_model_results_all.xlsx")
...
file_path <- paste0(Tables, "SA_copd_ps_model_death_all.docx")
...
#save as parquet
write_parquet(subset_df, "SA_copd_wave1_60d_iptw_all.parquet")
...
file_path <- file.path(Graphdir, "iptw_diagnostics", "SA_ps_dist_unweighted_all.png")
...
generate_density_plot(subset_df, "ate_weight_unstab", "Distribution of unstabilised ATE Weights by Treatment Group", "SA_dist_ate_unstab_all.png")
generate_density_plot(subset_df, "ate_weight_stab", "Distribution of stabilised ATE Weights Propensity Scores by Treatment Group", "SA_dist_ate_stab_all.png")
generate_density_plot(subset_df, "att_weight_unstab", "Distribution of ATT Weights by Treatment Group", "SA_dist_att_unstab_all.png")
generate_density_plot(subset_df, "att_weight_stab", "Distribution of ATT Weights by Treatment Group", "SA_dist_att_stab_all.png")
...
# Generate density plots
generate_density_plot(subset_df, "ate_weight_unstab", "Distribution of unstabilised ATE-Weighted Propensity Scores by Treatment Group", "SA_ps_dist_ate_unstab_all.png")
generate_density_plot(subset_df, "ate_weight_stab", "Distribution of stabilised ATE-Weighted Propensity Scores by Treatment Group", "SA_ps_dist_ate_stab_all.png")
generate_density_plot(subset_df, "att_weight_unstab", "Distribution of unstabilised ATT-Weighted Propensity Scores by Treatment Group", "SA_ps_dist_att_unstab_all.png")
generate_density_plot(subset_df, "att_weight_stab", "Distribution of stabilised ATT-Weighted Propensity Scores by Treatment Group", "SA_ps_dist_att_stab_all.png")
...
file_path <- file.path(Graphdir,  "iptw_diagnostics", "SA_weights_summary_all.xlsx")
...
all.equal(rownames(ate_weighted_table_unstab), rownames(ate_weighted_table_stab), rownames(att_weighted_table_unstab), rownames(att_weighted_table_stab), rownames(bal_unweighted_table))
...
#export this to excel
file_path <- file.path(Graphdir,  "iptw_diagnostics", "SA_smd_table_all.xlsx")
...
file_path <- file.path(Graphdir, "iptw_diagnostics", "SA_SMD_plot_all.png")
...
for (weight_var in weight_vars) {
  ...
  tab_data <- subset_df %>% dplyr::select(c("age_index", "gender", "bmicat", "eth", "imd", "diabetes_present", "hypertension_present","cvd_present", "allcancers_present", "past_asthma_present", "current_asthma_present", "kidney_present", "immunosuppression_present", "flu_vacc_present",  "pneumo_vacc_present",  "exacerb_present", "smokstatus", "treat", weight_var))
  ...
  file_path <- file.path(Graphdir, "iptw_diagnostics", paste0("SA_baseline_table_", weight_var, "_all.docx"))
