...
packages <- c("tidyverse", "MetBrewer", "arrow", "MatchThem", "mice", "parallelly", "furrr", "survey", "cobalt", "ggplot2", "twang", "ipw", "WeightIt", "gtsummary", "dplyr", "flextable", "openxlsx", "flexsurv", "survival", "jskm", "tidysmd", "sandwich", "survminer", "glmtoolbox", "openxlsx")
...
#determine distribution of missing_ons by treatment group, with % of deaths in the treatment group
df %>%
  group_by(treat) %>%
  summarise(missing_ons = sum(missing_ons),
            covid_death_present = sum(covid_death_present),
            total_death = sum(any_death_present == 1),
            total = n(),
            percent_missing = missing_ons/total,
            percent_covid_death = covid_death_present/total)

df$covid_death_present <- as.numeric(df$covid_death_present)

#code no missing deaths as covid
df$no_missing_covid <- as.numeric(df$covid_death_present)
df$no_missing_covid[df$missing_ons == 1] <- 0
df$covid_death_present <- df$no_missing_covid
...
censored_no_cause_death <- censored[,"% censored all-cause death"] * 100
...
write.xlsx(censored, file.path(Tables, "censored_table_no_missing_covid.xlsx"), rowNames = T)
...
outcomes <- c("no_missing_covid", "any_death_present")
time_to_outcome_vars <- c("timeinstudy3", "timeinstudy_death_any")
#check if timeinstudy3 is the same as timeinstudy_death_any
identical(subset_df$timeinstudy3, subset_df$timeinstudy_death_any)
#flag rows of data where timeinstudy3 is not equal to timeinstudy_death_any
subset_df$flag <- ifelse(subset_df$timeinstudy3 != subset_df$timeinstudy_death_any, 1, 0)
...
km_weighted <- function(weight_var, outcome, time_to_outcome_var) {
  ...
  file_path <- file.path(Graphdir, "cox_regression", paste0("km_", outcome, "_", weight_var, "_no_missing_covid.png"))
...
for (j in seq_along(outcomes)){
  ...
  file_path <- file.path(Graphdir, "cox_regression", paste0("schoenfeld_resid_",outcome_event,"_unadj_no_missing_covid.png"))
  png(file_path)
  ...
  file_path_resid <- file.path(Graphdir, "cox_regression", paste0("log_resid_", outcome_event, "_unadj_no_missing_covid.png"))
  png(file_path_resid)
  ...
  for (w in seq_along(weight_vars)) {
    ...
    file_path_iptw <- file.path(Graphdir, "cox_regression", paste0("schoenfeld_resid_", outcome_event, "_IPTW_", weight_name, "_no_missing_covid.png"))
    png(file_path_iptw)
    ...
    file_path_resid <- file.path(Graphdir, "cox_regression", paste0("log_resid_", outcome_event, "_IPTW_", weight_name, "_no_missing_covid.png"))
...
arrow::write_parquet(estimates, file.path(Tables, "QBA", "cox_log_regression_estimates_no_missing_covid.parquet"))
