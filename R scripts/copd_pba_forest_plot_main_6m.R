...
estimates_conventional <- read_parquet(file.path(Tables, "QBA", "SA_cox_log_regression_estimates_6m.parquet")) %>% dplyr::select(-contains("coef_"), -contains("se_"), -contains("res_p"), -contains("cox"), -contains("hr"), -contains("att_weight")) %>% filter(!stringr::str_detect(outcome_event, "any_"))

hosp_summary <- read_parquet(file.path(Tables, "QBA", "qba_hosp_summary_results_6m.parquet"))
hosp_record <- read_parquet(file.path(Tables, "QBA", "qba_hosp_record_results_6m.parquet"))
death_summary <- read_parquet(file.path(Tables, "QBA", "qba_death_summary_results_6m.parquet"))
death_record <- read_parquet(file.path(Tables, "QBA", "qba_death_record_results_6m.parquet"))

#load in death iptw dataset
D <- read_parquet(file.path(Datadir_copd, "SA_copd_wave1_6m_iptw.parquet"))
...
df <- read_parquet("copd_wave1_6m_hosp.parquet")
DH <- read_parquet("SA_copd_wave1_6m_iptw.parquet")[, c("patid", "ate_weight_stab")]
...
file_path_hosp <- file.path(Graphdir, "QBA", "forest_plot_full_main_6m.png")
