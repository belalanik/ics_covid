...
df <- read_parquet("SA_copd_wave1_6m_iptw.parquet")
...
write.xlsx(censored, file.path(Tables, "SA_censored_table_6m.xlsx"), rowNames = T)
...
km_weighted <- function(weight_var, outcome, time_to_outcome_var) {
  ...
  file_path <- file.path(Graphdir, "cox_regression", paste0("SA_km_", outcome, "_", weight_var, "_6m.png"))
...
for (j in seq_along(outcomes)){
  ...
  file_path <- file.path(Graphdir, "cox_regression", paste0("SA_schoenfeld_resid_",outcome_event,"_unadj_6m.png"))
  png(file_path)
...
arrow::write_parquet(estimates, file.path(Tables, "QBA", "SA_cox_log_regression_estimates_6m.parquet"))
