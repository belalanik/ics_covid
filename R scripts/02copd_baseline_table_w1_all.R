...
#MAIN TABLE
tab1 <- tbl_summary(subset_df %>%  dplyr::select(age_index, gender, bmicat, eth, smok, imd, diabetes_present, hypertension_present, cvd_present, allcancers_present, past_asthma_present, current_asthma_present, kidney_present, immunosuppression_present, flu_vacc_present, pneumo_vacc_present,  exacerb_present, treat),
                    by = treat,
                    label = list(age_index ~ "Age",
                                 ...
                                 current_asthma_present ~ "Current asthma",
...
if (file.exists(paste0(Tables, "copd_baseline_w1_60d_all.docx"))) {
  # Remove the file if it exists
  file.remove(paste0(Tables, "copd_baseline_w1_60d_all.docx"))
...
tab1 %>%
  as_flex_table() %>%
  flextable::save_as_docx(path = paste0(Tables, "copd_baseline_w1_60d_all.docx"), align = "left")
...
if (file.exists(paste0(Tables, "copd_outcomes_w1_60d_all.docx"))) {
  # Remove the file if it exists
  file.remove(paste0(Tables, "copd_outcomes_w1_60d_all.docx"))
  cat("File removed successfully.\n")
} else {
  cat("File does not exist.\n")
}
...
tab2 %>%
  as_flex_table() %>%
  flextable::save_as_docx(path = paste0(Tables, "copd_outcomes_w1_60d_all.docx"), align = "left")
