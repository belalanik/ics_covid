...
followup_total <- subset_df %>%
  summarise(median_timeinstudy2 = median(timeinstudy2),
            ...
            median_timeinstudy_covid_hes_any = median(timeinstudy_covid_hes_any))
...
followup <- add_row(followup, treat = "Total", median_timeinstudy2 = followup_total$median_timeinstudy2,
                    ...
                    median_timeinstudy_covid_hes_any = followup_total$median_timeinstudy_covid_hes_any)
colnames(followup) <- c("Treatment group", "Median follow-up COVID-19 hospitalisation", "Median follow-up COVID-19 death", "Median follow-up all-cause death", "Median follow-up COVID-19 hospitalisation (any)")
...
censored <- subset_df %>%
  ...
  summarise(prop_censored_timeinstudy2 = 1 - sum(timeinstudy2 == 183) / n(),
            prop_censored_timeinstudy3 = 1 - sum(timeinstudy3 == 183) / n(),
            prop_censored_timeinstudy_death_any = 1 - sum(timeinstudy_death_any == 183) / n(),
            prop_censored_timeinstudy_covid_hes_any = 1 - sum(timeinstudy_covid_hes_any == 183) / n())
...
censored <- add_row(censored, treat = "Total", prop_censored_timeinstudy2 = total_censored$prop_censored_timeinstudy2,
                    ...
                    prop_censored_timeinstudy_covid_hes_any = total_censored$prop_censored_timeinstudy_covid_hes_any)



#rename columns in censored
colnames(censored) <- c("Treatment group", "% censored COVID-19 hospitalisation", "% censored COVID-19 death", "% censored all-cause death", "% censored COVID-19 hospitalisation (any)")
...
censored_covid_hes_any <- censored[,"% censored COVID-19 hospitalisation (any)"] * 100
...
censored$"% censored COVID-19 hospitalisation (any)" <- censored[,"% censored COVID-19 hospitalisation (any)"] * 100
...
outcomes <- c("covid_hes_present", "covid_death_present", "any_death_present", "any_covid_hes_present")
time_to_outcome_vars <- c("timeinstudy2", "timeinstudy3", "timeinstudy_death_any", "timeinstudy_covid_hes_any")
...
for (j in seq_along(outcomes)){
  ...
  timeinstudy_var <- switch(outcome_position,
                            ...
                            as.name("timeinstudy_covid_hes_any"))
  ...
  outcome_label <- switch(outcome_position,
                          ...
                          as.name("COVID-19 hospitalisation (any)"))
  ...
  stopifnot(outcome_label != "COVID-19 hospitalisation (any)" || timeinstudy_var == "timeinstudy_covid_hes_any")
