/*=========================================================================
DO FILE NAME:		    10cr_copd_all_drugs_SA.do
...
DESCRIPTION OF FILE:	appends all inhaled medication files. Includes all patients in sensitivity analysis (asthma, no smoking, LTRA, other resp disease)
...
clear all 
capture log close
log using $Logdir/10cr_copd_all_drugs_SA.log , replace

cd $Datadir_copd

***append datasets containing individual prescriptions
use "${file_stub}_ics_single_clean_SA.dta"
gen ics_single = 1

append using "${file_stub}_ics_laba_clean_SA.dta", gen(ics_laba)
append using "${file_stub}_laba_single_clean_SA.dta", gen(laba_single)
append using "${file_stub}_laba_lama_clean_SA.dta", gen(laba_lama)
append using "${file_stub}_lama_single_clean_SA.dta", gen(lama_single)
append using "${file_stub}_triple_therapy_clean_SA.dta", gen(triple)
...
replace class = 5 if triple == 1

label define class 0 "ics_single" 1 "ics_laba" 2 "laba_single" 3 "laba_lama" 4 "lama_single" 5 "triple_therapy"
label values class class
drop ics_single ics_laba laba_single lama_single laba_lama 
drop termfromemis productname

compress
save "10cr_copd_all_drugs_SA.dta", replace
...
compress
save "copd_treatment_episodes_initiation_SA", replace
/************************************************************************
*************************************************************************
...
graph export $Graphdir/discontinuation60_any_SA.jpg, replace

compress
save "copd_treatment_episodes_60d_SA", replace
...
clear all
use "10cr_copd_all_drugs_SA.dta", replace
...
graph export $Graphdir/discontinuation6m_any_SA.jpg, replace

compress
save "copd_treatment_episodes_6m_SA", replace
