/*=========================================================================
DO FILE NAME:		    SB_04cr_copd_comorbidities.do
...
clear all
capture log close 
log using $Logdir/SB04cr_copd_comorbidities.log, replace
...
foreach comorbidity of local disease {
	foreach file of numlist 1/$no_Observation {
		...
		drop if eventdate <= td(01mar2020) | eventdate > td(01apr2021)
		...
		if `file' == 1{
			save "${file_stub}_Observation_`comorbidity'_w2.dta", replace
		}
		if `file' > 1{
			append using "${file_stub}_Observation_`comorbidity'_w2.dta"
		}
		compress
		save "${file_stub}_Observation_`comorbidity'_w2.dta", replace
	}
	
append "${file_stub}_Observation_`comorbidity'.dta"
duplicates drop
...
save "SB_${file_stub}_covariate_`comorbidity'.dta", replace
}
