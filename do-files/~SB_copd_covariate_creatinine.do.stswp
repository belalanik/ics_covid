/*=========================================================================
DO FILE NAME:	SB_copd_covariate_creatinine
...
capture log close
log using $Logdir/SB_covariate_creatinine_copd.log, replace
...
foreach file of numlist 1/$no_Observation {
	...
	merge m:1 patid using "SB_${file_stub}_Patid_list_included_all.dta", keep(match) nogen
    merge m:1 medcodeid using "$Codelistsdir/cl_creatinine.dta", keep(match) nogen
	if `file' == 1 {
		save "SB_${file_stub}_Observation_creatinine.dta", replace
	}
	if `file' > 1 {
		append using "SB_${file_stub}_Observation_creatinine.dta"
	}
	compress
	save "SB_${file_stub}_Observation_creatinine.dta", replace
}

use "SB_${file_stub}_Observation_creatinine.dta"
unique patid
...
destring numunitid, replace	
	...
	*******************************************************************************/
	...
	save "SB_${file_stub}_creatinine_values", replace
...
*******************************************************************************/
	...
	**************************************************************************/
	use "SB_${file_stub}_Patient_included_all.dta", clear
	keep patid gender yob covid_hes_date
	...
	merge 1:m patid using "SB_${file_stub}_creatinine_values.dta", nogen keep(match) force // only keep patients with test results available
...
*/
	...
	* save	
	...
	compress
	save "SB_${file_stub}_Observations_egfr", replace
...
save "SB_${file_stub}_covariate_kidney", replace
