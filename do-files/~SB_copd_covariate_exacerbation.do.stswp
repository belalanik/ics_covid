...
use "${file_stub}_DrugIssue_`product'.dta"
...
merge m:1 patid using "SB_copd_Patient_included_all.dta", keepusing(covid_hes_date) keep(match) nogen
...
collapse (max) annual_review abx ocs breathlessness cough sputum lrti aecopd, by(patid eventdate covid_hes_date)
...
save "SB_${file_stub}_covariate_exacerbation.dta", replace
