...
cap drop _merge
merge m:1 patid using "$Datadir_copd\\copd_Patient_included.dta", keepusing(enddate)
...
putexcel set "$Datadir_copd\treatment_changes_w1_60d_sankey_nomed_12m", replace
putexcel A1 = "source"
...
local target nomed
local source nomed
count if (`target'2 == 1 & `source'1[_n - 1] == 1 & patid == patid[_n - 1] | `source'1 == 1 & patid != patid[_n+1]  | `source'1 == 1 & date[_n+1] > td(`end') | _merge == 2) & enddate >= td(`end')
replace tag = 1 if (`target'2 == 1 & `source'1[_n - 1] == 1 & patid == patid[_n - 1] | `source'1 == 1 & patid != patid[_n+1]  | `source'1 == 1 & date[_n+1] > td(`end') | _merge == 2) & enddate >= td(`end')
return list
matrix stats = r(N)
putexcel C`row' = matrix(stats)
local row = `row' + 1
